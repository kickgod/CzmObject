...
namespace CzmWeb.App_Code
{
    public class GetDataFromView:PublicGetDataFromDB
    {
        ...
        /**************************   vwProjectInvestInfor     ***************************/
        /// <summary>
        /// 返回视图vwProjectInvestInfor里面所有的数据
        /// </summary>
        /// <returns></returns>
        public DataTable GetAllDataFrom_vwProjectInvestInfor()
        {
            SqlConnection con = GetSqlConnection();
            DataTable td = new DataTable();
            if (con.State != ConnectionState.Closed)
            ...
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT * FROM vwProjectInvestInfor";
            SqlDataAdapter adapter = new SqlDataAdapter(cmd);
            adapter.Fill(td);
            return td;
        }
        /// <summary>
        /// 返回视图vwProjectInvestInfor里面加上where条件的所有数据
        /// </summary>
        /// <param name="Where"></param>
        /// <returns></returns>
        public DataTable GetAllDataFrom_vwProjectInvestInfor(String Where)
        {
            SqlConnection con = GetSqlConnection();
            DataTable td = new DataTable();
            if (con.State != ConnectionState.Closed)
            ...
            if (Where != "")
            {
                cmd.CommandText = "SELECT * FROM vwProjectInvestInfor  Where " + Where;
            }
            else
            {
                cmd.CommandText = "SELECT * FROM vwProjectInvestInfor";
